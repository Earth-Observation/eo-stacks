...
call('mkdir -p ' + HOME + '/.local/share/applications/', shell=True) # Make folder for applications
call('mkdir -p ' + DESKTOP_PATH, shell=True) # Make folder for Desktop
...
call('printf "' + shortcut_metadata + '" > ' + HOME + '/.local/share/applications/jupyter.desktop', shell=True) # create a link in categories menu to your Jupyter notebook server
call('chmod +x ' + HOME + '/.local/share/applications/jupyter.desktop', shell=True) # Make executable

# Create Jupyter Lab Shortcut
url = 'http://localhost:8092' + "/lab" + token_parameter
shortcut_metadata = '[Desktop Entry]\nVersion=1.0\nType=Link\nName=Jupyter Lab\nComment=\nCategories=Development;\nIcon=' + ENV_RESOURCES_PATH + '/icons/jupyterlab-icon.png\nURL=' + url

call('printf "' + shortcut_metadata + '" > ' + HOME + '/.local/share/applications/jupyterlab.desktop', shell=True) # create a link in categories menu to your Jupyter Lab server
call('chmod +x ' + HOME + '/.local/share/applications/jupyterlab.desktop', shell=True) # Make executable
