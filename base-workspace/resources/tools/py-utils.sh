#!/usr/bin/env bash
PIPX_HOME=${1:-${PIPX_HOME:-"/usr/local/bin"}}
USERNAME=${2:-${NB_USER:-"jovyan"}}

echo "PIPX_HOME: $PIPX_HOME"
echo "USERNAME: $USERNAME"

DEFAULT_UTILS=("pylint" "flake8" "autopep8" "black" "yapf" "mypy" "pre-commit" "pydocstyle" "pycodestyle" "pytest" "pyupgrade" "safety" "coverage" "coverage-badge" "poetry2setup" "bandit" "pipenv" "virtualenv" "darglint" "isort" "supervisor" "poetry" "glances")
...
# ${pipx_path}pipx inject glances --system-site-packages --pip-args '--no-cache-dir --force-reinstall' "glances[action,browser,cloud,cpuinfo,docker,export,folders,gpu,graph,ip,raid,snmp,web,wifi]" 
